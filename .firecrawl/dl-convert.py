...
os.makedirs(".firecrawl/ref", exist_ok=True)
for i, h in enumerate(HASHES, 1):
    url = f"https://mir-s3-cdn-cf.behance.net/project_modules/1400_webp/{h}"
    stem = f"{i:02d}-" + h.removesuffix(".png")
    webp_path = f".firecrawl/ref/{stem}.webp"
    png_path = f".firecrawl/ref/{stem}.png"
    try:
        urllib.request.urlretrieve(url, webp_path)
...
print("\n".join(sorted(os.listdir(".firecrawl/ref"))))
