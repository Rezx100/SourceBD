...
export function normaliseAddressKey(input: string): string {
  ...
  // "mirpur-12," hide the extra-digit road/section conflict. Underscore is
  // a title/road separator (Village_Hemayetpur, Airport Road_East), not a
  // letter — leaving it glued village_hemayetpur as a competing village.
  s = s.replace(/[#().,:;/\-&_]/g, " ");
...
const EXTRA_PLACE_SKIP_ALIASES = [
  ...
  "naya",
  "noya",
  // Kind-of-place field heads and a common "south" misspelling. Not which place.
  "area",
  "space",
  "export",
  "shouth",
  // 2-letter honorifics. "Md. Airport East" never reaches [a-z]{3,} extra
  // mint otherwise, so airport≈airpark extraNameShare merges two roads.
  "md",
  "dr",
] as const;
...
function isExactPlaceSkipToken(place: string): boolean {
...
/** Compass/size stems used only for OCR peel (Kakhin ≈ Dakhin). Not which place. */
const EXTRA_PLACE_SKIP_FUZZY = [
  ...EXTRA_PLACE_SKIP_ALIASES.filter((t) => t.length >= 5),
  "dakhin",
  "dakshin",
  "south",
  "north",
...
function extraPlaceSkipToken(place: string): boolean {
  if (isExactPlaceSkipToken(place)) return true;
  if (HOUSING_CAMPUS_PLACES.has(place)) return false;
  if (VILLAGE_EXTRA_PLACES.has(place)) return false;
  if (ADMIN_TOKENS.has(place) && place !== "old" && place !== "new") return false;
  // Kakhin ≈ Dakhin. Cheap length+edit check only — extraNameShare on the
  // whole GENERIC set hung the fixture suite.
  if (place.length < 5) return false;
  for (const t of EXTRA_PLACE_SKIP_FUZZY) {
    if (t.length < 5 || Math.abs(place.length - t.length) > 1) continue;
    if (place === t || levenshtein(place, t) <= 1) return true;
  }
  return false;
}
...
function peelExtraPlaceSkip(
  ...
  for (let n = 0; n < 4; n++) {
    ...
    const nxt = rest.match(/^[\s_]*([a-z]{3,})\b/);
    if (!nxt) break;
    p = nxt[1]!;
...
const EXTRA_PLACE_SKIP_RE = (() => {
  ...
  for (const t of GENERIC_TOKENS) {
    if (isExactPlaceSkipToken(t)) skip.add(t);
  }
  for (const t of [
    "storied",
    "storey",
...
function isBuildingNameFollower(after: string): boolean {
  ...
  return Boolean(
    next &&
      /^(?:complex|tower|plaza|bhaban|bhawan|bahan|centre|center|market|court|building|chamber|mansion|housing)$/.test(
        next,
      ),
  );
}
...
function labelledBuildingHouseBefore(before: string): boolean {
  return /(?:building|bldg|house|hosue|flat|apartment|apt|unit)\s*(?:#|no\.?|number)?[\s.:-]*$/i.test(
    before,
  );
}

function extraDigitTruncatesLongerNumber(s: string, m: RegExpMatchArray): boolean {
  const end = (m.index ?? 0) + m[0].length;
  return /\d$/.test(m[0]) && /^\d/.test(s.slice(end));
}
...
function roadHoldingEntries(display: string): Array<{ digit: string; tail: string }> {
  ...
  // `_` is a word character, so `\s+road\b` never sees "Airport Road_East".
  // Treat underscore as a separator the way trailDirHit / titleSep already do.
  const roadGap = String.raw`(?:\s+|_)`;
  for (const word of ROAD_HOLDING_TAILS) {
    const comma = new RegExp(
      String.raw`${lead}${p}(\d{1,3})\s*,\s*${EXTRA_ROAD_SKIP_RE}([a-z][^,]{0,40}?)${roadGap}${word}(?![a-z])`,
      EXTRA_RE_FLAGS,
    );
    const space = new RegExp(
      String.raw`${lead}${p}(\d{1,3})${EXTRA_DIGIT_GLUE}${EXTRA_ROAD_SKIP_RE}([a-z][^,]{0,40}?)${roadGap}${word}(?![a-z])`,
      EXTRA_RE_FLAGS,
    );
    const intervening = new RegExp(
      String.raw`${EXTRA_DIGIT_LEAD}${p}(\d{1,3})\s*,\s*${EXTRA_PLACE_SKIP_RE}[a-z]{3,}(?![a-z])\s*,\s*${EXTRA_ROAD_SKIP_RE}([a-z][^,]{0,40}?)${roadGap}${word}(?![a-z])`,
      EXTRA_RE_FLAGS,
    );
    const interveningSpace = new RegExp(
      String.raw`${EXTRA_DIGIT_LEAD}${p}(\d{1,3})${EXTRA_DIGIT_GLUE}${EXTRA_PLACE_SKIP_RE}[a-z]{3,}(?![a-z])\s*,\s*${EXTRA_ROAD_SKIP_RE}([a-z][^,]{0,40}?)${roadGap}${word}(?![a-z])`,
      EXTRA_RE_FLAGS,
    );
...
function canonStreetWord(raw: string): string {
  ...
  if (raw === "st" || raw === "street" || raw === "streets") return "street";
  if (raw === "ave" || raw === "avenue" || raw === "avenues") return "avenue";
  if (raw === "blvd" || raw === "boulevard") return "boulevard";
...
function spelledOrdinalN(word: string): string | undefined {
  switch (word) {
    case "first":
      return "1";
    case "second":
...
function ordinalStreetEntries(display: string): Array<{ n: string; word: string }> {
  ...
  const numeric = new RegExp(
    String.raw`\b(\d{1,2})(?:st|nd|rd|th)[\s./_\u00AD\u200B\p{Pd}\u2212]*(${STREET_WORD_ALT})\b`,
    EXTRA_RE_FLAGS,
  );
  ...
  for (const m of s.matchAll(spelled)) {
    const n = spelledOrdinalN(m[1]!);
    if (!n) continue;
    out.push({ n, word: canonStreetWord(m[2]!) });
  }
  return out;
}

function unOrdinalStreetEntries(display: string): Array<{ word: string }> {
  const s = display.toLowerCase();
  const out: Array<{ word: string }> = [];
  const re = new RegExp(String.raw`\b(${STREET_WORD_ALT})\b`, EXTRA_RE_FLAGS);
  for (const m of s.matchAll(re)) {
    ...
    out.push({ word: canonStreetWord(m[1]!) });
  }
  return out;
}
...
function roadNameTokensFromTail(tail: string): string[] {
  ...
  for (let i = 0; i < raw.length; i++) {
    const t = raw[i]!;
    const nxt = raw[i + 1];
    // "D EPZ" is DEPZ. "C DA" is CDA. "I A" is IA. Do not drop the
    // single letter and then XOR DEPZ vs EPZ or IA vs A as two roads.
    if (t.length === 1 && nxt) {
      out.push(t + nxt);
      i += 1;
      continue;
    ...
    else if (t.length === 1 && !nxt) out.push(t);
  }
  return out;
}
...
function extraNameShare(a: string[], b: string[]): boolean {
  ...
  if (a.length === 1 && b.length === 1) {
    ...
    // Chanduddan vs Chand Uddan — the split second word is not a locality tail.
    if (
      long.startsWith(short) &&
      short.length >= 5 &&
      long.length - short.length >= 4 &&
      !GENERIC_TOKENS.has(long.slice(short.length)) &&
      !ADMIN_TOKENS.has(long.slice(short.length)) &&
      !isThoroughfareWord(long.slice(short.length))
    ) {
      return true;
    }
  }
  ...
  // Jajhor vs Jazor. Same stem, one letter dropped; Green vs Greene is
  // already sameWord.
  if (
  ...
  // Baro Bagh vs Barabagh — size adjective glued onto the same stem.
  if (a.length === 1 && b.length === 1) {
    const [short, long] = sa.length <= sb.length ? [sa, sb] : [sb, sa];
    const sizeAdj = new Set(["baro", "boro", "bara", "choto", "chhoto"]);
    const prefix = long.slice(0, long.length - short.length);
    if (prefix && long.endsWith(short) && sizeAdj.has(prefix)) return true;
  }
  // Chand Uddan vs Chanduddan — tokens of one side join to the other.
  if (a.length > 1 && a.join("") === sb) return true;
  if (b.length > 1 && b.join("") === sa) return true;
  return false;
}
...
/** Sheikh / Shaikh / Md / Dr / Doctor — not which road. Joydebpur is not this. */
...
function peelRoadDirectionPrefix(tokens: string[]): { dirs: string[]; rest: string[] } {
  const dirs: string[] = [];
  const rest = [...tokens];
  // Sheikh East Airport — skip the honorific so East peels. Do not strip
  // Joydebpur (not an honorific; KEEP East Joydebpur vs West Joydevpur).
  while (
    rest.length > 1 &&
    isSpacedHonorificCandidate(rest[0]!) &&
    rest
      .slice(1)
      .some((t) => isCompetingRoadStem(t) || ROAD_DIRECTION_KEEP.has(t))
  ) {
    rest.shift();
  }
...
function sameNamedRoadSpelling(a: string[], b: string[]): boolean {
  ...
  if (pa.dirs.length > 0 && pb.dirs.length > 0) {
    ...
    if (ra && rb) {
      ...
      // Sheikh Airport East vs Shaikh Airport West — honorific spelling
      // makes ra!==rb, but both competing stems are airport. Do not let
      // sheikh≈shaikh license East vs West. Joydebpur vs Joydevpur has no
      // competing stem and still coreSameNamedRoadSpelling KEEP.
      const restStemA = competingRoadStemOf(pa.rest);
      const restStemB = competingRoadStemOf(pb.rest);
      if (
        restStemA &&
        restStemB &&
        isCompetingRoadStem(restStemA) &&
        isCompetingRoadStem(restStemB) &&
        coreSameNamedRoadSpelling(restStemA, restStemB) &&
...
function initialismVsLongName(a: string[], b: string[]): boolean {
  ...
  if (!sa || !sb) return false;
  const [short, long] = sa.length <= sb.length ? [sa, sb] : [sb, sa];
  return short.length <= 2 && long.length >= 5;
}
...
function isLocalityTailPlace(place: string): boolean {
  if (place.length >= 7 && place.endsWith("kur")) return true;
  return (PLACE_TAILS as readonly string[]).some(
    (tail) => place.length > tail.length + 3 && place.endsWith(tail),
  );
}

function isVillageishPlace(place: string): boolean {
  if (place.length >= 7 && place.endsWith("kur")) return true;
  return (PLACE_TAILS as readonly string[]).some(
    (tail) => place.length > tail.length && place.endsWith(tail),
  );
}
...
/** Green Street src=road at House 10 vs leftover Greenwood leftover-digit /
 *  same house digit. Not a global src=road mixed-digit XOR. */
function leftoverCompetingPrefixConflict(
  ...
  const xPrefix = x.places.some((p) => leftoverCompetingStemPrefix(p));
  const yPrefix = y.places.some((p) => leftoverCompetingStemPrefix(p));
  const xStem = x.places.some((p) => isCompetingRoadStem(p));
  const yStem = y.places.some((p) => isCompetingRoadStem(p));
  return (
    (xStem && yPrefix && !xPrefix) ||
    (yStem && xPrefix && !yPrefix) ||
    (xPrefix && yPrefix && x.places.join("\0") !== y.places.join("\0"))
  );
}
...
function leftoverCompetingVsNamedRoadConflict(
  x: { places: string[]; src: ExtraSrc },
  y: { places: string[]; src: ExtraSrc },
): boolean {
  if (sameNamedRoadSpelling(x.places, y.places)) return false;
  const xComp = extraLooksLikeCompetingRoad(x);
  const yComp = extraLooksLikeCompetingRoad(y);
  if (xComp === yComp) return false;
  if (x.src === "road" && yComp && !xComp) return true;
  if (y.src === "road" && xComp && !yComp) return true;
  return false;
}

/** "10, C DA Road" vs "10, DA" — DA is not minted as a 2-letter extra. */
function unsuffixedInitialismLeftover(
  road: {
    digit: string;
    places: string[];
    src: ExtraSrc;
...
function extraNamedOnOtherNonHousing(places: string[], other: Candidate): boolean {
  ...
  // Every token of the extra, not a leftover substring (Nazrul of
  // Kazi Nazrul Islam Avenue vs leftover Nazrul next to Green).
  return (
    names.length > 0 &&
    names.every((p) =>
      other.tokens.some(
...
function isKeptRoadDirectionToken(token: string): boolean {
  return ROAD_DIRECTION_KEEP.has(token) && !/^(?:naya|noya)$/.test(token);
}

/** Every KEEP compass in a skip run, including glued Northeast / Southeast.
 *  lastKeptRoadDirection alone collapsed North East Airport and South East
 *  Airport to the same East Airport extra. */
function keptRoadDirections(text: string): string[] {
  const dirs: string[] = [];
  const toks = text.toLowerCase().split(/[^a-z]+/).filter(Boolean);
  for (const t of toks) {
    if (isKeptRoadDirectionToken(t)) {
      dirs.push(t);
      continue;
    }
    const glued = peelConcatenatedDirections(t);
    if (glued.dirs.length === 0) continue;
    for (const d of glued.dirs) {
      if (isKeptRoadDirectionToken(d)) dirs.push(d);
    }
    if (glued.rest && isKeptRoadDirectionToken(glued.rest)) dirs.push(glued.rest);
  }
  return dirs;
}

function lastKeptRoadDirection(text: string): string | null {
  const dirs = keptRoadDirections(text);
...
function leftoverPeeledMintable(
  ...
  // Fashion Plaza leftover Green / leftover Greenwood. Court / Tower / Market
  // are not building-landmark heads; leftover-mint still so leftover XOR can
  // split leftover Green vs leftover Greenwood after the building. Airport
  // Plaza already leftover-mints via afterAirportFamilyThoroughfare.
  if (
    afterBuildingLandmark &&
    names.some((n) => isCompetingRoadStem(n) || leftoverCompetingStemPrefix(n))
  ) {
    return true;
  }
  // After Circular / Nazrul / Mirpur / Dit / Airport Link leftover-mint Green
  // AND Greenwood so leftover XOR can split them. Greenwood is a competing-stem
  // prefix, not isCompetingRoadStem, so the Issue-remainder gate never
  // leftover-minted it. Leftover-minting Green after Nazrul must still XOR
  // Green vs Nazrul (KEEP Plot # 10, 10 Kazi Nazrul Islam Avenue, Green vs
  // Plot # 10, 10 Green, Nazrul). Do not leftover-mint multi-token leftover
  // PRIMARY (International Airport) — names.every already refuses that.
  if (
    afterThoroughfare &&
    names.some((n) => isCompetingRoadStem(n) || leftoverCompetingStemPrefix(n))
  ) {
...
function appendLeftoverIssueRoadExtras(
  ...
  let afterBuildingLandmark = false;
...
function bareProperRoadPlaces(
  ...
  const push = (
    ...
    const next =
      nextRaw.match(/^([a-z]{2,})(?![a-z])/)?.[1] ??
      nextRaw.match(/^[\s,./_\u00AD\u200B\p{Pd}\u2212]+([a-z]{2,})(?![a-z])/u)?.[1];
    const skipDirs = keptRoadDirections(skipped);
    const skipDir = skipDirs.length > 0 ? skipDirs[skipDirs.length - 1]! : null;
    const compassPlace = isKeptRoadDirectionToken(place);
    const compassNext = Boolean(next) && isKeptRoadDirectionToken(next!);
    ...
    if (
      /^(?:suite|suit|room|shop|house|hosue|holding|hold|plot|plots|building|bldg|flat|apartment|apt|unit|ward|block|sector|section|floor|dag|dug)$/.test(
        place,
      )
    ) {
      return;
    ...
    // Honorific + compass + competing stem in any token order (Sheikh East
    // Airport, East Sheikh Airport, Sheikh Airport East, stacked North East).
    // CompassPlace+next used to drop the stem. Do not restore a global
    // prefixed XOR (East Joydebpur vs West Joydevpur stays one premises).
    const seq: string[] = [place];
    {
      let rest = after;
      for (let n = 0; n < 6; n++) {
        const hit = rest.match(
          /^[\s,./_\u00AD\u200B\p{Pd}\u2212]+([a-z]{2,})(?![a-z])/u,
        );
        if (!hit) break;
        const tok = hit[1]!;
        if (/^(?:union|village|vill)$/.test(tok)) break;
        if (ADMIN_TOKENS.has(tok) || HOUSING_CAMPUS_PLACES.has(tok)) break;
        if (isThoroughfareWord(tok)) break;
        if (/^(?:plot|plots|holding|house|hosue)$/.test(tok)) break;
        seq.push(tok);
        rest = rest.slice(hit[0].length);
      }
    }
    const stemIdx = seq.findIndex((t) => isCompetingRoadStem(t));
    if (stemIdx >= 0) {
      const stem = seq[stemIdx]!;
      const before = seq.slice(0, stemIdx);
      const afterStem = seq.slice(stemIdx + 1);
      const honorifics = before.filter((t) => isSpacedHonorificCandidate(t));
      const dirsBefore = [
        ...skipDirs,
        ...before.filter((t) => isKeptRoadDirectionToken(t)),
      ];
      const dirsAfter = afterStem.filter((t) => isKeptRoadDirectionToken(t));
      const leftoverBefore = before.filter(
        (t) => !isSpacedHonorificCandidate(t) && !isKeptRoadDirectionToken(t),
      );
      if (
        leftoverBefore.length === 0 &&
        (honorifics.length > 0 || dirsBefore.length > 0 || dirsAfter.length > 0)
      ) {
        out.push({
          digit: String(Number(digit)),
          places: [...honorifics, ...dirsBefore, stem, ...dirsAfter],
        });
        return;
      }
    }
    ...
    if (skipDirs.length > 0 && !compassPlace) {
      if (next && /^(?:union|village|vill)$/.test(next)) return;
      if (compassNext) {
        out.push({ digit: String(Number(digit)), places: [...skipDirs, place, next!] });
        return;
      }
      out.push({ digit: String(Number(digit)), places: [...skipDirs, place] });
      return;
    }
    if (compassNext) {
      out.push({ digit: String(Number(digit)), places: [place, next!] });
    ...
    // when the sequence walker did not already mint (leftover before stem).
    if (
      next &&
      ...
      const consumed = nextRaw.replace(
        new RegExp(
          String.raw`^[\s,./_\u00AD\u200B\p{Pd}\u2212]*${next.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}(?![a-z])`,
          "u",
        ),
        "",
      );
      const trail =
        consumed.trimStart().match(/^([a-z]{3,})(?![a-z])/)?.[1] ??
        consumed.match(/^[\s,./_\u00AD\u200B\p{Pd}\u2212]+([a-z]{3,})(?![a-z])/u)?.[1];
      const trailCompass = Boolean(trail) && isKeptRoadDirectionToken(trail!);
      out.push({
        digit: String(Number(digit)),
  ...
  for (const m of s.matchAll(
    new RegExp(
      String.raw`${EXTRA_DIGIT_LEAD}((?:no\s*[:.\-]?|number|#)\s*)?(\d{1,3})${EXTRA_DIGIT_UNIT}(${EXTRA_DIGIT_GLUE})(${EXTRA_PLACE_SKIP_RE})([a-z]{3,})(?![a-z])`,
      EXTRA_RE_FLAGS,
    ),
  )) {
  ...
  for (const m of s.matchAll(
    new RegExp(
      String.raw`${PLOT_DIGIT_LEAD}(\d{1,3})\s*,\s*(${EXTRA_PLACE_SKIP_RE})([a-z]{3,})(?![a-z])`,
      EXTRA_RE_FLAGS,
    ),
  )) {
    if (extraDigitTruncatesLongerNumber(s, m)) continue;
  ...
  // last comma field (Dhaka); walk each field and skip housing/admin.
  const skipPrefix = new RegExp(String.raw`^${EXTRA_PLACE_SKIP_RE}`, "u");
  const restatedDigit = new RegExp(
    String.raw`^(?:(?:no\s*[:.\-]?|number|#)\s*)?\d{1,3}${EXTRA_DIGIT_UNIT}$`,
    "u",
  );
  const labelledField =
    /^(?:house|hosue|holding|hold|plot|plots|building|bldg|flat|apartment|apt|unit|ward|block|sector|section|floor|dag|dug|suite|suit|room|shop|area|space|export|level)\b/;
  const restatedPlotList = new RegExp(
    String.raw`^(?:&|and)?\s*(?:(?:no\s*[:.\-]?|number|#)\s*)?\d{1,3}(?:\s*[&/,]\s*\d{1,3})*$`,
    "u",
  );
  const otherPlotHead = new RegExp(
    String.raw`^(?:(?:no\s*[:.\-]?|number|#)\s*)?(\d{1,3})\b`,
    "u",
  );
  for (const m of s.matchAll(
    new RegExp(
      String.raw`${EXTRA_DIGIT_LEAD}((?:no\s*[:.\-]?|number|#)\s*)?(\d{1,3})${EXTRA_DIGIT_UNIT}`,
      EXTRA_RE_FLAGS,
    ),
  )) {
    if (extraDigitTruncatesLongerNumber(s, m)) continue;
    if (labelledBuildingHouseBefore(s.slice(0, m.index ?? 0))) continue;
    if (labelledWardSectorDigit(s, m)) continue;
    const digit = String(Number(m[2]!));
    if (out.some((e) => e.digit === digit)) continue;
    ...
    for (let fi = 0; fi < afterFields.length; fi++) {
      ...
      let trimmed = rawField.replace(/^[\s./_\u00AD\u200B\p{Pd}\u2212]+/u, "");
      if (!trimmed) continue;
      trimmed = trimmed.replace(/^\([^)]*\)\s*/u, "");
      if (!trimmed) continue;
      ...
      if (labelledField.test(trimmed)) continue;
      const otherPlot = otherPlotHead.exec(trimmed);
      if (otherPlot && String(Number(otherPlot[1]!)) !== digit) break;
      if (/\b(?:estate|bscic|epz|cepz|kepz|depz|university|industrial)\b/.test(trimmed)) {
        seenCampus = true;
        continue;
      }
        ...
        if (
          later.some((field) => {
            ...
            if (
              afterLaterBuilding &&
              leftoverPeeledMintable(
                peelLeftoverIssueRoad(afterLaterBuilding),
                true,
                false,
                false,
                false,
      ...
      if (
        /^(?:union|mouza|mouja|thana|upazila|upazilla|village|vill|post|gpo)(?![a-z])/.test(
          trimmed,
        )
      ) {
        allowMint = true;
        const kind = trimmed.match(
          /^(union|mouza|mouja|thana|upazila|upazilla|village|vill|post|gpo)(?![a-z])/,
        )![1]!;
        const rest = trimmed
          .replace(
            /^(?:union|mouza|mouja|thana|upazila|upazilla|village|vill|post|gpo)(?![a-z])[\s./_\u00AD\u200B\p{Pd}\u2212]*/u,
            "",
          )
          .replace(/^(?:(?:of|the)\s+)+/u, "");
      ...
      if (/^(?:no\.?|number|#)\s+(?!\d)/.test(trimmed)) {
        trimmed = trimmed.replace(/^(?:no\.?|number|#)\s+/u, "");
        allowMint = true;
      }
      ...
      const pm = skipped.match(/^([a-z]{3,})\b/);
      if (!pm) continue;
      const place = pm[1]!;
      ...
      // A second skip is a thana after a village (Sastapur, Narayanganj,
      // Fatullah) — stop rather than mint the later thana as a road extra.
      if (
        HOUSING_CAMPUS_PLACES.has(place) ||
        ADMIN_TOKENS.has(place)
      ) {
      ...
      // "Ford Nagar" is the village clause. Do not walk on to the thana
      // (Singair) as a second extra.
      if (next && (PLACE_TAILS as readonly string[]).includes(next)) {
        break;
      }
      // Holding # 83, Rupganj — labelled skip only. First loops / leading
...
function namedExtraTails(
  ...
  for (const p of leadingVillageExtras(display)) {
    ...
    // Do not drop Fatehabad just because Hathazari was also minted.
    if (
      bare.some(
        (b) => b.digit === p.digit && extraNameShare(b.places, p.places),
      )
    ) {
      continue;
    }
...
function primaryTitledNamesFromDisplay(display: string): string[] {
  ...
  const titleBound = String.raw`(?<![a-z])${title}(?![a-z])`;
  ...
  // `_` is a word character, so `\bunion\b` misses Union_Telulzora.
  const lead = new RegExp(
    String.raw`(?<![a-z]{3,}${titleSep}+)(?:the\s+)?${titleBound}${titleSep}*,?\s*(?:(?:of|the)\s+)*([a-z]{3,})(?![a-z])`,
    EXTRA_RE_FLAGS,
  );
  for (const m of s.matchAll(lead)) push(m[1]!);
  const trail = new RegExp(
    String.raw`([a-z]{3,})${titleSep}+${titleBound}`,
    EXTRA_RE_FLAGS,
  );
...
function unionNamesFromDisplay(display: string): string[] {
  ...
  const afterLead = new RegExp(
    String.raw`(?:the\s+)?(?<![a-z])${title}(?![a-z])[\s./_\u00AD\u200B\p{Pd}\u2212]*,?\s*(?:(?:of|the)\s+)*[a-z]{3,}(?![a-z])((?:[\s,./_\u00AD\u200B\p{Pd}\u2212]+[a-z]{3,}(?![a-z]))*)`,
    EXTRA_RE_FLAGS,
  );
...
function hasVillageKindTitle(display: string): boolean {
  const s = rewriteHouseOffice(display).toLowerCase();
  return new RegExp(String.raw`(?:the\s+)?(?<![a-z])(?:village|vill)(?![a-z])`, "u").test(s);
}
...
function extraRoadPlaceConflict(a: Candidate, b: Candidate): boolean {
  ...
  // minted, so the pair loop never sees it.
  for (const x of ea) {
    if (unsuffixedInitialismLeftover(x, b.display)) return true;
  }
  for (const y of eb) {
  ...
  for (const x of ea) {
    for (const y of eb) {
      ...
      // not license two English roads (Valuka on both of Airport vs Green).
      // Run this before both-road XOR so named-road spellings at a shared
      // village still merge. Gulshan/Banani housing is already excluded.
      {
        const housingish = (p: string) =>
          HOUSING_CAMPUS_PLACES.has(p) || ADMIN_TOKENS.has(p);
      ...
      // BSCIC + Konabari). Airport vs Joydebpur is still two roads.
      if (x.src === "road" && y.src === "road") {
        if (initialismVsLongName(x.places, y.places)) {
          // B.B vs Bangabandhu
        } else if (extraLooksLikeCompetingRoad(x) || extraLooksLikeCompetingRoad(y)) {
          return true;
        } else if (
          x.places.length === 1 &&
          y.places.length === 1 &&
          (x.places[0]!.length <= 3 || y.places[0]!.length <= 3)
        ) {
          return true;
        }
      }
      ...
      if (xRoadish && yRoadish) {
        ...
        // vs "Green, Airport" stay two rows. Sowdagor Lane vs CDA is
        // multi-word vs a short extra and falls through to every().
        if (initialismVsLongName(x.places, y.places)) {
          const shortBare =
            (x.src === "road" &&
              y.src === "bare" &&
              y.places.join("").length <= 3) ||
            (y.src === "road" &&
              x.src === "bare" &&
              x.places.join("").length <= 3);
          if (shortBare) return true;
          continue;
        }
        if (x.places.length === 1 && y.places.length === 1) {
          if (extraLooksLikeCompetingRoad(x) && extraLooksLikeCompetingRoad(y)) {
            return true;
          }
          if (
          ...
          if (x.src === "road" && y.src === "bare" && extraLooksLikeCompetingRoad(y)) {
            return true;
          }
          if (y.src === "road" && x.src === "bare" && extraLooksLikeCompetingRoad(x)) {
            return true;
          }
          if (
            x.src === "road" &&
          ...
          // Plot 397 Joydebpur Road, Chandona vs Plot 397 Chandona — the
          // village leftover is already named on the road string.
...
export function extraHoldingsConflict(addressA: string, addressB: string): boolean {
  return extraHoldingConflict(
    candidateFor(cleanAddressString(addressA)),
    candidateFor(cleanAddressString(addressB)),
  );
}
