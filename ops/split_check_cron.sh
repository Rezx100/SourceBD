...
# abort the script silently. Same reasoning as conflation_check_cron.sh.
set -uo pipefail

# Run from the SourceBD VPS cron, for example:
# 23 3 * * * /opt/sourcebd/ops/split_check_cron.sh >> /opt/sourcebd/etl/logs/split_check.log 2>&1
#
# Daily is the right cadence, mirroring the conflation check: a split is
# created by a scraper run and stays put until somebody merges it, so catching
# it within a day is enough, and a quieter check is a check that still gets read.
...
echo "=== $(date -Is) supplier split check ==="
...
output=$(docker compose run --rm --entrypoint python \
  ...
  ops/check_supplier_splits.py --quiet 2>&1)
rc=$?
[ -n "$output" ] && echo "$output"

case "$rc" in
  0) ;;
  1) notify ":rotating_light: SourceBD: one legal entity is split across multiple published supplier profiles with fragmented evidence. Repair with ops/merge_duplicate_suppliers.py (dry run first)." ;;
  ...
  *) notify ":warning: SourceBD: supplier split check could not run (exit ${rc})." ;;
esac
