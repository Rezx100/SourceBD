...
1. Lands on marketing site → sees live "verified suppliers" counter + SBI methodology.
...
4. Opens supplier profile → SBI score breakdown, partner factories, media gallery, reviews, contact (gated by plan).
5. Sends RFQ / message → tracks conversation → optionally tracks order to port.

## Features in scope (v1)
- Public marketing site (sourcebd.com) with live counter, SBI methodology, compliance education.
- Buyer app: Dashboard, Smart Match, Discover, Factory profile, Buying-house profile, Messages, Order Tracking, Compliance Hub, RFQ Manager, Settings.
- Supplier portal: claim profile, dashboard, edit profile, manage incoming RFQs, upload certs/photos.
- Admin panel: supplier CRUD, bulk CSV import, score recalculation, certification verification queue, sanctions queue, user management.
- SBI scoring engine (4 pillars; +UFLPA traceability bonus for US buyers).
...
- ≥ 90% of suppliers have computed SBI scores.
