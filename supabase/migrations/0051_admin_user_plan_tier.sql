-- 0051 — extend admin_user_update whitelist with plan_tier.
--
-- Background. Migration 0031 added profiles.plan_tier with CHECK
-- (plan_tier in ('starter','growth','enterprise')). Migration 0041
-- shipped admin_user_update with a 3-key whitelist (role,
-- is_suspended, suspended_reason). Admins therefore could not change
-- a buyer's plan from the /admin/users/[id] surface — the only place
-- plan is mutable today is the buyer's own /app/settings, which is
-- wrong: plan changes must be admin-driven until Stripe ships.
--
-- This migration replaces admin_user_update with a 4-key whitelist
-- adding 'plan_tier'. Same enum check, same self-edit guard, same
-- last-admin guard, same single-tx audit-log write. No data change.
--
-- Reversibility: re-apply 0041's CREATE OR REPLACE for admin_user_update.
...
comment on function public.admin_user_update(uuid, jsonb) is
  'Spec A5 + 0051 — admin-only user mutation. Whitelist: role, '
  'is_suspended, suspended_reason, plan_tier. Self-edit refused (42501). '
  'Last-admin guard (P0001). One admin_audit_log row per call with full '
  'old->new diff, single tx.';
