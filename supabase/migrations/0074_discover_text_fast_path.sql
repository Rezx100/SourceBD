-- Discover text-search fast path for PostgREST (~3s statement timeout).
-- supplier_search_candidates + full search_rank still exceeds REST limits for
-- common product queries even after 0073 FTS pooling. Discover text loads now
-- use a direct FTS-first suppliers scan with lightweight ranking; Smart Match
-- keeps supplier_search_candidates.
...
comment on function public.discover_suppliers(
  ...
  'Spec B1 Discover RPC. Browse uses direct scan; text queries use FTS-first lightweight ranking for PostgREST latency.';
