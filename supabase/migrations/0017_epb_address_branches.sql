-- =============================================================================
-- Migration 0017: EPB address branches in v_supplier_addresses_direct
--
-- Audit (2026-05-22) showed 43 EPB-only suppliers with 0 rows in
-- v_supplier_addresses despite 100% of EPB source_records (472/472)
-- carrying `epb_factory_address` and `epb_office_address`. The view as
-- recreated by migration 0016 has no EPB branch — same shape of bug as
-- the BGAPMEA gap fixed by 0016. The EPB scraper writes both addresses
-- via `etl/scrapers/epb_web.py::_build_record` and surfaces them as
-- `epb_factory_address` (factory site) and `epb_office_address`
-- (registered HQ). This migration adds two EPB-specific branches
-- (factory + registered) reading those payload keys.
--
-- Phone/email policy: EPB payload carries no phone/email keys (the EPB
-- Exporter Database public surface does not publish these), so both
-- branches leave phone/email NULL. Contact details for EPB-tagged
-- suppliers must come from BGMEA/BKMEA/BTMA/BGAPMEA enrichment.
--
-- Trust tier: EPB is Tier 1 (gov register, _TIER_MAP). Primary source.
--
-- No writes to base tables. Pure view recreation, additive over 0016.
-- Reversibility: re-running 0016 restores the prior shape.
-- =============================================================================
...
-- EPB (Spec / migration 0017): epb_factory_address (factory site) and
...
comment on view public.v_supplier_addresses_direct is
  'Raw, source-records-driven verified addresses per supplier (BGMEA factory/mailing, BKMEA factory/mailing, RSC factory, BTMA factory/mailing/registered, BGAPMEA factory/registered, EPB factory/registered). Subset of v_supplier_addresses; the parent view UNIONs this with inherited rows for RSC sibling factories.';

-- v_supplier_addresses: direct ∪ inherited (RSC sibling factories). Recreated
-- verbatim from migration 0016 — dropping v_supplier_addresses_direct above
