-- 0095 — buyer_supplier_profile facility docs + certs (REZ-93 / B0b).
--
-- WHY
-- ---
-- RSC inspects buildings, not companies. When A2 unpublishes an attached
-- facility, that building's mirrored inspection PDFs (and site-specific
-- certifications) vanish with the row — buyers see fewer facts than exist for
-- the site. No view or function previously referenced facility_of on the
-- read path.
--
-- WHAT
-- ----
-- Extends `buyer_supplier_profile` documents AND certifications CTEs:
--   * parent's own rows unchanged;
--   * plus rows from live children where facility_of = parent.id;
--   * each inherited row carries building_name = facility.company_name
--     (Extension / Unit-2 suffix preserved — that suffix IS the building id);
--   * parent's own rows omit building_name (payload keys byte-identical when
--     facility_of is empty — production is still 0 attached);
--   * dedupe by row id (same row twice → once). Same kind/doc_type across
--     buildings is NOT collapsed — those are distinct ids.
--
-- DISPLAY-ONLY (founder decision 6 Aug 2026)
-- ------------------------------------------
-- Inherited certifications are profile display only. They must NOT feed:
--   * discover_suppliers p_cert_kinds (filters `c.supplier_id = s.id`);
--   * registry pills (v_supplier_registry_ids join on s.id only);
--   * t13_source_count / receipts (source_records tiers, untouched here).
-- Do not "fix" that by wiring facility_of into discover or pills.
--
-- RSC remediation progress cards stay parent-only; per-building RSC is
-- REZ-73's Facilities section.
--
-- PII
-- ---
-- Inheritance stays inside this SECURITY DEFINER RPC's curated jsonb keys.
-- No facility slug, id, contact, email, or phone is emitted. Unpublished
-- facility rows remain unreadable via PostgREST RLS on compliance_documents
-- / certifications; only this join surfaces them, and only when facility_of
-- points at the published mother being viewed.
--
-- NON-GOALS
-- ---------
-- No re-point of compliance_documents / certifications.supplier_id.
-- No facility attach (B1). No numeric roll-up (REZ-92).
--
-- REVERSE
-- -------
-- Re-apply the docs CTE from 20260725_rez_security_hardening_2.sql
-- (parent-only join on compliance_documents).
--
...
comment on function public.buyer_supplier_profile(text) is
  ...
  'discover/pills/t13). partner_factories[] on buying_house pages and '
  ...
  'Never emits facility slug or id.';
