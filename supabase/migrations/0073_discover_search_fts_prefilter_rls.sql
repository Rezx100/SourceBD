-- Discover text-search hotfix + RLS lockdown for internal tables.
--
-- Root cause: supplier_search_candidates scanned every published supplier and
-- computed full search_rank for each row. Text queries like "denim" exceeded
-- PostgREST statement_timeout (~4s) intermittently. Migration 0071 fixed the
-- empty-query browse path only; this adds a GIN-backed candidate pool for
-- text queries before ranking.
--
-- Also enables RLS on internal tables flagged by Supabase Advisor. ETL and
-- SECURITY DEFINER RPCs retain access via service_role / function owner.
...
as $fn$
  ...
  search_pool as materialized (
    select s.id
      from public.suppliers s
      cross join query_flags qf
     where qf.q is not null
       ...
       and exists (
         select 1
          ...
          where c.supplier_id = s.id
            ...
            and replace(c.kind::text, '_', ' ') ilike qf.q_like
       )
    union
    select distinct s.id
      from public.suppliers s
      cross join query_flags qf
      join public.source_records srq
        on srq.supplier_id = s.id and srq.status = 'active'
  ...
  filtered as materialized (
    select s.id, s.slug, s.company_name, s.entity_type::text as entity_type,
     ...
     where s.is_published = true
       ...
       and (
         qf.q is null
         or exists (select 1 from search_pool sp where sp.id = s.id)
       )
...
comment on function public.supplier_search_candidates(
  ...
  'Internal shared supplier search brain. Text queries narrow via discover_search_tsv GIN pool before ranking. SECURITY DEFINER; do not grant to app roles.';
...
-- Internal / ETL tables — deny direct anon/authenticated API reads.
alter table public.rsc_industry_metrics enable row level security;
alter table public.rsc_monthly_reports enable row level security;
alter table public.content_drafts enable row level security;
alter table public.sanctions_list_entries enable row level security;

-- Search taxonomy — reference data, but not public API surface.
alter table public.search_concept_groups enable row level security;
