-- Discover cards: expose primary verified address so City/Dist labels match
-- the full supplier profile header (longest address on file, not factory-only
-- suppliers.city / suppliers.district when HQ and mill diverge).

create or replace function public.supplier_primary_address(p_supplier_id uuid)
returns text
language sql
stable
  ...
  select coalesce(
    (
      select va.address
       ...
       where va.supplier_id = p_supplier_id
         and coalesce(btrim(va.address), '') <> ''
       order by length(va.address) desc
       limit 1
    ),
    (
      select s.address_raw
...
comment on function public.supplier_primary_address(uuid) is
  'Longest verified address for a supplier; falls back to suppliers.address_raw.';

revoke all on function public.supplier_primary_address(uuid) from public;
  ...
  if v_q is null then
    return query
      ...
      select p.id, p.slug, p.company_name, p.entity_type, p.city, p.district,
             ...
             public.supplier_primary_address(p.id) as primary_address,
             p.total_count
        from page p;
    return;
  end if;
...
comment on function public.discover_suppliers(
  ...
  'Spec B1 Discover RPC. Returns primary_address for card/profile location parity.';
